...
int skipCol;				// number of columns to skip (passed as argv)
unsigned int burnin;		// number of accepted values that are discarded (burnin period)
unsigned int subsampling;	// only every 'subsampling' accepted value (after the burnin period) is recorded
unsigned int set;			// number of samples that form a set; every set the ESS is calculated (MIGHT BE DROPPED STILL)
unsigned int noSets;		// number of sets
double jumpCSD;				// scale parameter for Cauchy distribution (passed as argv), that is the proposal distribution for 'c'
double jumpRSD;				// standard deviation for Gaussian distribution (passed as argv), that is the proposal distribution for 'r'
std::string _initialRC;		// name of data file for initialising the growth rates 'r' and initial population sizes 'c'
...
double minESS = 1000000000;					// minimum ESS calculated from all samples and all parameters
double minESS_C = 1000000000;				// minimum ESS calculated from all samples of C
double minESS_R = 1000000000;				// minimum ESS calculated from all samples of R
double minESS_logL = 1000000000;			// minimum ESS calculated from all samples of logL
double minHD_C = 2;
double maxHD_C = -1;
double minHD_R = 2;
double maxHD_R = -1;
...
bool print_logLTS;		// switches for output (logLTS)
bool print_ess;			// switches for output (summary)
bool print_output;		// switches for output (output)
bool outliersPresent;	// outliers in input file
bool timeHours;
...
// core functions
extern void readcommandlineArguments(int argc, char *argv[]);
...
void readcommandlineArguments(int argc, char *argv[])
{
	...
	if (argc != 17) // if input is incomplete stop
	{
	...
	fileIdentifier = argv[2];
	skipCol = atoi (argv[3]);
	outliersPresent = atoi (argv[4]);
	burnin = atoi (argv[5]);
	subsampling = atoi (argv[6]);
	noSets = atoi (argv[7]);
	set = atoi (argv[8]);
	jumpCSD = atof (argv[9]);
	jumpRSD = atof (argv[10]);
	_initialRC = argv[11];
	print_logLTS = atoi (argv[12]);
	print_ess = atoi (argv[13]);
	print_output = atoi (argv[14]);
	timeHours = atoi (argv[15]);
	seed = atol (argv[16]);
	if (seed <= 0) seed = rdtsc(); 	// if seed <= 0 a random number seed is created automatically based on computer run time
...
void checkArgvInput()
{
	...
	if(outliersPresent != 0 && outliersPresent != 1)
	{
		std::cout << "Error: outliersPresent = " << outliersPresent << ". See ReadMe file for input arguments.\n";
		exit(1);
	}
	...
	if(print_logLTS != 0 && print_logLTS != 1)
	{
		std::cout << "Error: print_logLTS = " << print_logLTS << ". See ReadMe file for input arguments.\n";
		exit(1);
	}

	if(print_ess != 0 && print_ess != 1)
	{
		std::cout << "Error: print_ess = " << print_ess << ". See ReadMe file for input arguments.\n";
		exit(1);
	}

	if(print_output != 0 && print_output != 1)
	{
		std::cout << "Error: print_output = " << print_output << ". See ReadMe file for input arguments.\n";
		exit(1);
	}

	if(timeHours != 0 && timeHours != 1)
	{
		std::cout << "Error: timeHours = " << timeHours << ". See ReadMe file for input arguments.\n";
		exit(1);
	}
}
...
void printparameters(std::ofstream * outfile)
{
	...
	*outfile << "\n#No Sets " << noSets;
	*outfile << "\n#Set " << set;
	*outfile << "\n#Total number accepted values (wo burn-in) "  << noSets*set*subsampling ;
	*outfile << "\n#Outliers detected ";
	if(outliersPresent)
	{
		*outfile << "Yes";
