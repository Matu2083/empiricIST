...
for (( j=1; j<=$maxIndex; j++ ))
	do
		tail -n+16 $prefixFileName-$j-$suffixFileName\_Diag_R.txt > MCMC-$j-DFE_Diag_R.txt
		numLines=$(cat MCMC-$j-DFE_Diag_R.txt | wc -l)
		protIDV=( $(cat MCMC-$j-DFE_Diag_R.txt | cut -f1) )
		refprotID=${protIDV[0]}
		head -n 1 MCMC-$j-DFE_Diag_R.txt | tail -1 | cut -f1,3- > temp_$refprotID.txt
				...
				if [ ${protIDV[$((i - 1))]} != -1 ]
					...
					if [ $refprotID != ${protIDV[$((i - 1))]} ]
					 	then
							...
							head -n $i MCMC-$j-DFE_Diag_R.txt | tail -1 | cut -f1,3- > temp_$refprotID.txt
					fi
			 fi	
			done
	done
...
# If there are 'old' auxiliary files (Sorted_Data_Diag_R_*) delete them
if [ $(ls Sorted_Data_Diag_R_* 2>delete.txt | wc -l) != 0 ]
	then
		rm Sorted_Data_Diag_R_*
	fi
...
for (( j=1; j<=$iterations; j++ ))
	do
		...
		cat $(echo $files) > Sorted_Data_Diag_R_$j.txt
	done

# Paste all individual files and create a single file. 	
files=$(eval "echo Sorted_Data_Diag_R_{1..$iterations}.txt") 
cat FirstLineDiagR.txt $(echo $files) > $suffixFileName\_Diag_R.txt

# Move final file to the original folder and delete the auxiliary folder
mv $suffixFileName\_Diag_R.txt ../
cd ..
rm -r RDiag
