...
# It takes 4 input arguments ('pathToData' 'prefixFileName' 'suffixFileName' 'maxIndex') and returns a single file that contains all diagnostics for all sampled initial population sizes for all mutants
...
for (( j=1; j<=$maxIndex; j++ ))
	do
		tail -n+16 $prefixFileName-$j-$suffixFileName\_Diag_C.txt > MCMC-$j-DFE_Diag_C.txt
		numLines=$(cat MCMC-$j-DFE_Diag_C.txt | wc -l)
		protIDV=( $(cat MCMC-$j-DFE_Diag_C.txt | cut -f1) )
		refprotID=${protIDV[0]}
		newIndex=1
		head -n 1 MCMC-$j-DFE_Diag_C.txt | tail -1 | cut -f3- > temp1_$refprotID-1.txt
			...
			do
				if [ ${protIDV[$((i - 1))]} != -1 ]
					...
					if [ $refprotID == ${protIDV[$((i - 1))]} ]
					 	then
			 				((newIndex++))
							head -n $i MCMC-$j-DFE_Diag_C.txt | tail -1 | cut -f3- > temp1_$refprotID-$newIndex.txt	 	
...
# If there are 'old' auxiliary files (Sorted_Data_Diag_C_*) delete them
if [ $(ls Sorted_Data_Diag_C_* 2>delete.txt | wc -l) != 0 ]
	then
		rm Sorted_Data_Diag_C_*
	fi
...
for (( j=1; j<=$iterations; j++ ))
	do
		...
		cat $(echo $files) > Sorted_Data_Diag_C_$j.txt
	done

# Paste all individual files and create a single file. 	
files=$(eval "echo Sorted_Data_Diag_C_{1..$iterations}.txt")
cat FirstLineDiagC.txt $(echo $files) > $suffixFileName\_Diag_C.txt


# Move final file to the original folder and delete the auxiliary folder
mv $suffixFileName\_Diag_C.txt ../
cd ..
rm -r CDiag
